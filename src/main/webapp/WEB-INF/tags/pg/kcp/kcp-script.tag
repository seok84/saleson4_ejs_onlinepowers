...
function kcpLaunchWebStandard(){
	...
	for(var i=0, length = OrderHandler.getTotalItems().length;i<length;i++){
		good_info += "seq=" + (i+1) + chr31 + "ordr_numb=" + orderCode + chr31 + "good_name=" + OrderHandler.getTotalItems()[i].itemName + chr31 + "good_cntx=" + OrderHandler.getTotalItems()[i].quantity
			+ chr31 + "good_amtx=" + OrderHandler.getTotalItems()[i].saleAmount;
		if(i+1 != length)
			good_info += chr30;
	}
	...
	try {		 
		...
		if($('#escrowStatus').val() != null && $('#escrowStatus').val() != 'N'){
			...
			document.order_info.bask_cntx.value = OrderHandler.getTotalItems().length;
